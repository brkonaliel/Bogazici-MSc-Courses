function make_predictions_selected_features()    
...
%     [img_names_ayahoo_test, img_classes_ayahoo_test, bbox_ayahoo_test, attributes_ayahoo_test] = read_att_data(fullfile(project_path, 'attribute_data', 'ayahoo_test.txt'));
    ...
    for i=1:64
        % load trained models
        temp = load(fullfile(pwd, 'models_selected_features', strcat('model_apascal_', int2str(i))));
        model_apascal = temp.model;
        temp = load(fullfile(pwd, 'models_selected_features', strcat('model_hog_', int2str(i))));
        model_hog = temp.model;
        temp = load(fullfile(pwd, 'models_selected_features', strcat('model_texton_', int2str(i))));
        model_texton = temp.model;
        temp = load(fullfile(pwd, 'models_selected_features', strcat('model_color_', int2str(i))));
        model_color = temp.model;
        
        % get predictions and probabilities
        attribute_test = attributes_apascal_test(:,i)*2-1;
        temp = load(fullfile(pwd, 'selected_features', strcat('model_apascal_', int2str(i))));
        apascal_features = temp.uniqued;
        [labels, ~, ~] = predict(attribute_test, sparse(apascal_test(:, apascal_features)), model_apascal, '-b 1');
        early_fusion_labels(:, i) = labels;
        temp = load(fullfile(pwd, 'selected_features', strcat('model_hog_', int2str(i))));
        apascal_hog_features = temp.uniqued;
        [labels, ~, hog_prob] = predict(attribute_test, sparse(hog_apascal_test(:, apascal_hog_features)), model_hog, '-b 1');
        hog_labels(:, i) = labels;
        temp = load(fullfile(pwd, 'selected_features', strcat('model_texton_', int2str(i))));
        apascal_texton_features = temp.uniqued;
        [labels, ~, texton_prob] = predict(attribute_test, sparse(texton_apascal_test(:, apascal_texton_features)), model_texton, '-b 1');
        texton_labels(:, i) = labels;
        temp = load(fullfile(pwd, 'selected_features', strcat('model_color_', int2str(i))));
        apascal_color_features = temp.uniqued;
        [labels, ~, color_prob] = predict(attribute_test, sparse(color_apascal_test(:, apascal_color_features)), model_color, '-b 1');
        ...
        % calculate rocs
        scores_early_fusion = apascal_test(:, apascal_features)*model_apascal.w';
        temp = computeROC(scores_early_fusion, attribute_test);
        roc_early_fusion(i) = temp.area;
        scores_hog = hog_apascal_test(:, apascal_hog_features)*model_hog.w';
        temp = computeROC(scores_hog, attribute_test);
        roc_hog(i) = temp.area;
        scores_texton = texton_apascal_test(:, apascal_texton_features)*model_texton.w';
        temp = computeROC(scores_texton, attribute_test);
        roc_texton(i) = temp.area;
        scores_color = color_apascal_test(:, apascal_color_features)*model_color.w';
...
%         break;
    ...
    save(fullfile(pwd, 'predictions_selected_features', 'early_fusion_labels'), 'early_fusion_labels');
    save(fullfile(pwd, 'predictions_selected_features', 'late_fusion_labels'), 'late_fusion_labels');
    save(fullfile(pwd, 'predictions_selected_features', 'hog_labels'), 'hog_labels');
    save(fullfile(pwd, 'predictions_selected_features', 'color_labels'), 'color_labels');
    save(fullfile(pwd, 'predictions_selected_features', 'texton_labels'), 'texton_labels');
    save(fullfile(pwd, 'predictions_selected_features', 'roc_early_fusion'), 'roc_early_fusion');
    save(fullfile(pwd, 'predictions_selected_features', 'roc_hog'), 'roc_hog');
    save(fullfile(pwd, 'predictions_selected_features', 'roc_color'), 'roc_color');
    save(fullfile(pwd, 'predictions_selected_features', 'roc_texton'), 'roc_texton');
    save(fullfile(pwd, 'predictions_selected_features', 'roc_late_fusion'), 'roc_late_fusion');
